...
- Без surface `RenderingDevice` работает с одним frame-in-flight, поэтому режим
  подходит для correctness-тестов, GPU-картинки и скриншотов, но его frame timing
  нельзя сравнивать с обычным оконным запуском или Quest. Перф проекта по-прежнему
